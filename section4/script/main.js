...
const animationTimeline = () => {
    ...
    // Show "Next" button after animation is complete
    const nextBtn = document.getElementById("nextBtn");
    nextBtn.style.opacity = 0; // Initially hidden
    nextBtn.style.pointerEvents = "none"; // Disable click while hidden

    // Fade in "Next" button after the animation ends (custom delay)
    tl.to(nextBtn, 1, { opacity: 1, y: 0, delay: 6 }); // Fade in the button after 6 seconds
    nextBtn.style.pointerEvents = "auto"; // Enable click after it appears

    // Redirect to the next page (section5)
    nextBtn.addEventListener("click", () => {
        window.location.href = "../section5/index.html"; // Redirect to section5
    });
}
